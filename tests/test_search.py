...
class SearchSqlTests(unittest.TestCase):
    ...
    def test_partner_count_and_workspace_cnpj_filters_are_applied(self):
        ...
        self.assertIn("FROM rfb_partners partner_count_filter", sql)
        self.assertIn("partner_count_filter.partner_count>=%s", sql)
        self.assertIn("partner_count_filter.partner_count<=%s", sql)
        self.assertEqual(sql.count("FROM rfb_partners partner_count_filter"), 1)
        self.assertIn(">=%s", sql)
        self.assertIn("<=%s", sql)
