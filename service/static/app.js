...
async function loadBillingSummary() {
  ...
  try {
    ...
    const cyclePayments = data.payments?.length ? `<section class="section-card billing-orders"><div class="section-card-heading"><div><span class="eyebrow">CICLOS E RECARGAS</span><h2>Histórico financeiro</h2><p>Cada cobrança fica registrada separadamente, inclusive renovações.</p></div></div><div class="billing-order-list">${data.payments.map((payment) => `<article><span class="billing-order-status status-${escapeHtml(payment.status)}">${escapeHtml(paymentLabels[payment.status] || payment.status)}</span><div><strong>${escapeHtml(payment.plan_name)}</strong><small>${payment.due_date ? `Vencimento ${formatBillingDate(payment.due_date)}` : formatDate(payment.updated_at)} · ${payment.amount_cents === null ? "Valor em conferência" : (Number(payment.amount_cents) / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}${payment.billing_type ? ` · ${escapeHtml(payment.billing_type === "CREDIT_CARD" ? "Cartão" : payment.billing_type)}` : ""}</small></div><strong class="billing-cycle-credit">${payment.credits_granted ? `+${Number(payment.credits_granted).toLocaleString("pt-BR")} créditos` : "—"}</strong></article>`).join("")}</div></section>` : "";
